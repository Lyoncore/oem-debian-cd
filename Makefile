...
ifdef FORCENONUSONCD1
	...
	$(Q)if [ -x "/usr/sbin/debootstrap" -a _$(INSTALLER_CD) != _1 ]; then \
		...
		/usr/sbin/debootstrap --arch $(ARCH) --print-debs $(CODENAME) /tmp/debian-cd-debootstrap file://$(MIRROR) $(DEBOOTSTRAP)/$(CODENAME)-$(FULLARCH) \
...
ifeq ($(CDIMAGE_INSTALL_BASE),1)
	...
	$(Q)for DISK in $(FIRSTDISKS); do \
	    ...
	    if [ -x "/usr/sbin/debootstrap" ]; then \
	        ok=yes; \
		mkdir -p /tmp/debian-cd-debootstrap ; \
	        for p in `/usr/sbin/debootstrap --arch $(ARCH) --print-debs $(CODENAME) /tmp/debian-cd-debootstrap file://$(MIRROR) $(DEBOOTSTRAP)/$(CODENAME)-$(FULLARCH)`; do \
